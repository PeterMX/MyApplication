...
import kotlinx.android.synthetic.main.activity_mmsc.*

class MMSC : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_mmsc)
        title = "M/M/S/C"
        supportActionBar?.subtitle = "Población C y Servidores S"
        ...
        return when (item.itemId) {
            R.id.calcular -> {
                if(editText3.text.isNotEmpty() && editText5.text.isNotEmpty() && editText4.text.isNotEmpty() && editText6.text.isNotEmpty()){
                    ...
                    siguiente.putExtra("tipo",4)//Tipo cuatro significa que es MMSC
